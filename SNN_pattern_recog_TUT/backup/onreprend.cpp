...
int main(int ac, char* av[])
{
	...
	sprintf(strbuf, "%s/%d.1.stimtimes", dir.c_str(), sys->mpi_rank());
	stimtimefile = strbuf;
	sprintf(strbuf, "%s/%d.2.stimtimes", dir.c_str(), sys->mpi_rank());
	stimtimefile2 = strbuf;
	...
	StimulusGroup * n_stim = new StimulusGroup(nb_stim, stimtimefile, STIMFILE);
	...
	StimulusGroup * n_stim2 = new StimulusGroup(nb_stim, stimtimefile2, STIMFILE);
	...
	sprintf(strbuf, "%s/%d.2.pact", dir.c_str(), sys->mpi_rank());
	PatternMonitor * patmon = new PatternMonitor(
		n_exc2, string(strbuf) , monfile2.c_str(), 100
	);
	sprintf(strbuf, "%s/%d.e2e2.wmon", dir.c_str(), sys->mpi_rank());
	WeightMonitor * wmone2e2 = new WeightMonitor(
		con_e2e2, string(strbuf), 1
	);
	sprintf(strbuf, "%s/%d.ee.wmon", dir.c_str(), sys->mpi_rank());
	WeightMonitor * wmonee = new WeightMonitor(
		con_ee, string(strbuf), 1
	);
	wmonee->add_equally_spaced(10);
	sprintf(strbuf, "%s/%d.ee2.wmon", dir.c_str(), sys->mpi_rank());
	WeightMonitor * wmonee2 = new WeightMonitor(
		con_ee2, string(strbuf), 1
	);
	wmonee2->add_equally_spaced(10);
	// Load stim
	if(!stimfile1.empty()){
