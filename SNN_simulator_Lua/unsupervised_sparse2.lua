...
model:add(nn.L1Penalty(lambda))
local sparsity = 0.05
local transfer = nn.SparseCriterion(sparsity, 3, nil, false) --nn[opt.activation]() -- save the transfer layer for later data extraction
--model:add(nn.SparseCriterion(0.05))
model:add(transfer)
...
-- Calculate average activation for hidden layer
local sparsityLoss
do
  local dataset = ds:trainSet()
  local batch = dataset:batch(dataset:nSample())
  local input = batch:inputs():input()

  model:forward(input)

  transfer._avgAct = torch.sum(transfer.output, 1):squeeze() / dataset:nSample()
  local sparsities = torch.ones(hiddenSize):fill(sparsity)
  sparsityLoss  = torch.sum(sparsity * torch.log(torch.cdiv(sparsities, transfer._avgAct)) + (1 - sparsity) * torch.log(torch.cdiv(1 - sparsities, 1 - transfer._avgAct)))
  print('norm of average activation')
  print(torch.norm(transfer._avgAct))
end
...
local criterion = nn.MSECriterion() --nn.MultiCriterion()
--criterion:add(nn.MSECriterion())
--criterion:add(nn.SparseCriterion(0.05))

local oldback = criterion.updateOutput
criterion.updateOutput = function(self, input, target)
  oldback(self, input, target)
  self.output = self.output + sparsityLoss
  return self.output
end

train = dp.Optimizer{
   acc_update = opt.accUpdate,
   loss = nn.ModuleCriterion(criterion, nil, nn.Convert()),
...
xp = dp.Experiment{
   ...
   observer = {
      ...
      dp.EarlyStopper{
         ...
         save_strategy = { save = function() end, verbose = function() end, setup = function() end }
      },
      ad
   },
   random_seed = os.time(),
