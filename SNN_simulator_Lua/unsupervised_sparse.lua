...
-- Hotfixes
nn.Decorator.updateOutput = function(self, input, target)
  self.output = self.module:updateOutput(input[1]:double(), input[2]:double())
  return self.output
end

local olduo = nn.MSECriterion.updateOutput
nn.MSECriterion.updateOutput = function(self, input, target)
  return olduo(self, input, target)
end

unsup.AutoEncoder.updateOutput = function(self, input, target)
  self.encoder:updateOutput(input)
  self.output = self.decoder:updateOutput(self.encoder.output)
  self.loss:updateOutput(self.decoder.output, target)
  if self.lambda then
     self.codecost:updateOutput(self.encoder.output)
  end
  return self.output
end

nn.Sequential.updateOutput = function(self, input, target)
   local currentOutput = input
   for i=1,#self.modules do
      currentOutput = self:rethrowErrors(self.modules[i], i, 'updateOutput', currentOutput, target)
   end
   self.output = currentOutput
   return currentOutput
end

local oldugi = nn.Convert.updateGradInput
nn.Convert.updateGradInput = function(self, input, gradOutput)
--  print(input, gradOutput)
  return oldugi(self, input[1], gradOutput)
end
require 'feedback/imagefeedback'
require 'criterion/sparsecriterion'
...
encoder:add(nn.Convert(ds:ioShapes(), 'bf')) -- to batchSize x nFeature (also type converts)
...
local model = unsup.AutoEncoder(encoder, decoder, 1, nil, 1, nn.SparseCriterion(0.05))--nn.Sequential()
--model:add(nn.Convert(ds:ioShapes(), 'bf')) -- to batchSize x nFeature (also type converts)
--model:add(unsup.AutoEncoder(encoder, decoder, 1, nil, 1, nn.SparseCriterion(0.05)))
...
for i = 1, datasize do
  model:forward(mnist_train[1][i]:reshape(28*28):double(), mnist_train[2][i])
