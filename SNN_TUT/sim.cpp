/*
	...
	-	one responding to mnist stimulus
	-	the other to the corresponding labels and to the previous population
	MNIST -> POP1 -> POP2 <- LABELS
	-> : stimulates

	Compile the program with the Makefile :
	$> make
	it outputs an exectuable file : sim

	Run it with mpirun :
	$> mpirun -n 4 ./sim --argument1 --argument2
	-n 4 : run the executable with the 4 cores of the processor

	Arguments you can pass through the console :
	--on 1000 : runs the simulation for 1000 seconds
	--pat : activate mnist stimulation
	--stim : activate labels stimulation
	--init : intial burn of 10s without plasticity actived
	--save : save the state at the end of the simulation
	--load : load the previously saved state
	--rank 3: prints 3 before the name of every files generated by the simulation,
		that way you can track and save the results from different simulations
	--ras : generate a binary spike raster used for the find_patterns script
...
int main(int ac, char* av[])
{
	...
	//	destinations for the different ressources
	...
	//	neuron & synapses constants
	float poisson_rate = 1.0;
	float tau_hom = 5;
	float eta_rel = 0.1;
	float kappa = 1.0; // target Hz rate of STDP triplet
	float weight = 0.2;
